...
class EvidenceCoverageMetric(BaseMetric):
    ...
    def calculate(self, context: MetricContext) -> MetricResult:
        if (
            not context.answer
            or context.documents is None
            or len(context.documents) == 0
        ):
            return MetricResult(
    ...
    def calculate(self, context: MetricContext) -> MetricResult:
        ...
        try:
            ...
            return MetricResult(
                ...
                details={"error": str(e), "sentences_checked": len(all_sentences)},
...
class EpistemicConsistencyMetric(BaseMetric):
        ...
        def run_async_isolated():
            ...
            try:
                res, cost = loop.run_until_complete(
                    self._generate_samples_async(context)
                )
    ...
    async def _generate_samples_async(self, context: MetricContext):
        ...
        system_prompt = "You are a helpful assistant. Answer the question based on the provided documents strictly (if provided), otherwise use your general knowledge."

        tasks = [
            self.service.llm_call_async(
                system_prompt=system_prompt, prompt=prompt, temperature=float(temp)
            )
            for temp in temps
        ]
    ...
    def _calculate_similarities(
        self, samples: List[str], context: MetricContext
    ) -> List[float]:
    ...
    def _format_result(
        self, score: float, samples: List[str], std: float, ci_95: float, cost: float
    ) -> MetricResult:
    ...
    def _create_stable_result(self) -> MetricResult:
        return MetricResult(
            ...
            details={"explanation": "Assumed stable."},
            execution_metadata={"total_cost_usd": 0.0},
        )
    ...
    def _create_unreliable_result(self, cost: float) -> MetricResult:
        return MetricResult(
            ...
            details={"explanation": "No valid samples."},
            execution_metadata={"total_cost_usd": cost},
        )
...
class SourceDiversityMetric(BaseMetric):
    def calculate(self, context: MetricContext) -> MetricResult:
        ...
        label, explanation = self.threshold_evaluator.evaluate_diversity(
            normalized_score
        )
    ...
    def _count_relevant_documents(self, context: MetricContext) -> int:
        ...
        for doc_emb in context.document_embeddings:
            try:
                ...
                logger.exception(
                    f"Error calculating similarity for document embedding: {e}"
                )
                continue
    ...
    def _verify_with_llm(
        ...
        if not response or not response.get("response"):
            return SpanCheckResult(
                ...
                0,
                response.get("cost", 0.0) if response else 0.0,
            )
